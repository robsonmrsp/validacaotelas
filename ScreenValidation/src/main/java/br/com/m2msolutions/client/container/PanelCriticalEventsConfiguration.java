...
public class PanelCriticalEventsConfiguration extends FormPanel {
	...
	private CheckBoxListView<DtoCriticalEvent> listCriticalEvents;
	...
	protected void populateListCriticalEvents(DtoCategory category) {
		eventsConfigurationService.getCriticalEventsByCategory(category, new AsyncCallback<ArrayList<DtoCriticalEvent>>() {
			@Override
			...
			@Override
			public void onSuccess(ArrayList<DtoCriticalEvent> criticalEvents) {
				updateListCriticalEventsContent(criticalEvents);
			}
		});
	}

	protected void updateListCriticalEventsContent(ArrayList<DtoCriticalEvent> criticalEvents) {
		listCriticalEvents.getStore().removeAll();
		listCriticalEvents.getStore().add(criticalEvents);
	}

	private CheckBoxListView<DtoCriticalEvent> getListCriticalEvents() {
		if (listCriticalEvents == null) {
			listCriticalEvents = new CheckBoxListView<DtoCriticalEvent>();
		...
		if (buttonOk == null) {
			...
			buttonOk.addSelectionListener(new SelectionListener<ButtonEvent>() {
				...
				public void componentSelected(ButtonEvent ce) {
					// TODO deverá tambem alterar o estado da tela widgetde
